...
absl::Status
ReadInput(const string input_filename, RangesAndIngredients &input)
{
    ...
    while (getline(file, line))
    {
        ...
        if (reading_ranges)
        {
            ...
            RETURNERRORIF(
                ...
                absl::StatusCode::kUnknown, "error reading range line: " + line);
            long int start, end;
            RETURNERRORIF(
                !absl::SimpleAtoi(range_start_end[0], &start),
                absl::StatusCode::kInvalidArgument,
                "error converting to int: " + range_start_end[0]);
            RETURNERRORIF(
                !absl::SimpleAtoi(range_start_end[1], &end),
                absl::StatusCode::kInvalidArgument,
        ...
        else
        {
            ...
            RETURNERRORIF(
                !absl::SimpleAtoi(line, &ingredient_id),
                absl::StatusCode::kInvalidArgument,
...
void part2(const RangesAndIngredients &input)
{
    ...
    for (const Range &range : ranges)
    {
        ...
        if (current >= start)
        {
            // we already counted the ingrediesnt from start to current, advance start
            start = current + 1;
        }
        if (start > end)
        {
        ...
        // cout << "adding from " << start << " to " << end << " (" << end - start + 1 << ")" << endl;
