...
absl::Status ReadInput(const string input_filename, vector<Range>& ranges) {
    ...
    RETURNERRORIF(!getline(file, line), absl::StatusCode::kNotFound,
                  "error reading input");
    ...
    for (const string& part : input_parts) {
        ...
        RETURNERRORIF(range_start_end.size() != 2, absl::StatusCode::kUnknown,
                      "error reading input");
        long int start, end;
        RETURNERRORIF(!absl::SimpleAtoi(string(range_start_end[0]), &start),
                      absl::StatusCode::kInvalidArgument,
                      "error reading line: " + range_start_end[0]);
        RETURNERRORIF(!absl::SimpleAtoi(string(range_start_end[1]), &end),
                      absl::StatusCode::kInvalidArgument,
...
sequence of digits repeated twice. So, 55 (5 twice), 6464 (64 twice), and 123123
(123 twice) would all be invalid IDs.

Your job is to find all of the invalid IDs that appear in the given ranges. In
the above example: Adding up all the invalid IDs in this example produces
1227775554.
    ...
    for (const Range& range : ranges) {
        for (long int id = range.first; id <= range.second; id++) {
            ...
            for (int substr_len = 1; substr_len <= half_len; substr_len++) {
                ...
                // check if each (at least 1) chunk of size substr_len is the
                // same
                ...
                for (int chunk = 1; all_good && chunk < num_chunks; chunk++) {
                    ...
                    string current_chunk_substr =
                        id_as_str.substr(start_idx, substr_len);
...
int main() {
    ...
    if (!res_input.ok()) {
        std::cout << "error reading input from " << kInputFilename
                  << "; Error: " << res_input << endl;
        return -1;
    }
    part1(input);
    part2(input);
