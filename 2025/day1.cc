...
absl::Status ReadInput(const string input_filename,
    ...
    while (getline(file, line)) {
        bool left = line[0] == 'L';
        int steps;
        RETURNERRORIF(!absl::SimpleAtoi(line.substr(1, line.size()), &steps),
                      absl::StatusCode::kInvalidArgument, "error reading line")
...
void part1(const vector<Instruction>& input) {
    ...
    for (const Instruction& instruction : input) {
        int increment =
            instruction.left ? -instruction.steps : instruction.steps;
