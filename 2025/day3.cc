...
absl::Status ReadInput(const string input_filename, vector<vecInt>& inputs) {
    ...
    while (getline(file, line)) {
        ...
        for (const char digit : line) {
            int digit_as_int;
            RETURNERRORIF(!absl::SimpleAtoi(string(1, digit), &digit_as_int),
...
void part2(const vector<vecInt>& batteries) {
    ...
    for (const vecInt& battery : batteries) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            int largest = -1;
            int largest_idx = -1;
            // find largest idx between last largest, leaving space for at least
            // the remaining digits
            for (int j = last_largest_idx + 1;
                 j < (battery.size() - remaining_digits); j++) {
